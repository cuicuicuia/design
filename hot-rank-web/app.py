...
async def init_pg_pool():
    try:
        pool = await asyncpg.create_pool(
            ...
            command_timeout=60,
            server_settings={
                'application_name': 'hot-rank-web'
            }
        )
        # 测试连接是否可用
        async with pool.acquire() as conn:
            await conn.fetchval('SELECT 1')
        logging.info(f"PostgreSQL连接池初始化成功: {PG_HOST}:{PG_PORT}/{PG_DB}")
        return pool
    except Exception as e:
        logging.error(f"PostgreSQL连接失败: {e}")
        logging.error(f"连接配置: host={PG_HOST}, port={PG_PORT}, user={PG_USER}, database={PG_DB}")
        logging.error("请检查:")
        logging.error("1. PostgreSQL是否正在运行")
        logging.error("2. 连接配置是否正确 (检查 config.py)")
        logging.error("3. 如果使用Docker, 运行: docker ps 查看容器状态")
        raise
...
async def get_data(item_id: str):
    ...
    try:
        ...
        # 检查 card_table 是否存在
        card_table_data = await redis_client.get("card_table")
        if not card_table_data:
            logging.error("card_table not found in Redis. Please run: python tools/pushSomethings.py")
            raise HTTPException(
                status_code=500, 
                detail="card_table not initialized in Redis. Please run: python tools/pushSomethings.py"
            )
        table_dict = json.loads(card_table_data)
